...
export const Inventory: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal open={opsOpen} onOpenChange={setOpsOpen}><ModalContent className="max-w-2xl"><ModalHeader><ModalTitle>{opsMode === 'receive' ? 'Receive Stock' : opsMode === 'adjust' ? 'Adjust Stock' : 'Transfer Stock'}</ModalTitle></ModalHeader><div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4"><div className="grid gap-2 md:col-span-2"><label className="text-xs font-bold uppercase text-muted-foreground">Variant</label><select className="h-11 rounded-xl border border-border/60 bg-background px-3 text-sm" value={opsForm.variant_id} onChange={(e) => setOpsForm({ ...opsForm, variant_id: e.target.value })}><option value="">Select variant</option>{variants.map((v) => <option key={v.id} value={v.id}>{v.sku} • {v.product_name}</option>)}</select></div>{opsMode !== 'transfer' && <div className="grid gap-2"><label className="text-xs font-bold uppercase text-muted-foreground">Warehouse</label><select className="h-11 rounded-xl border border-border/60 bg-background px-3 text-sm" value={opsForm.warehouse_id} onChange={(e) => setOpsForm({ ...opsForm, warehouse_id: e.target.value })}><option value="">Select warehouse</option>{warehouses.filter((w) => w.is_active).map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}</select></div>}{opsMode === 'transfer' && <div className="grid gap-2"><label className="text-xs font-bold uppercase text-muted-foreground">From</label><select className="h-11 rounded-xl border border-border/60 bg-background px-3 text-sm" value={opsForm.from_warehouse_id} onChange={(e) => setOpsForm({ ...opsForm, from_warehouse_id: e.target.value })}><option value="">Select source</option>{warehouses.filter((w) => w.is_active).map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}</select></div>}{opsMode === 'transfer' && <div className="grid gap-2"><label className="text-xs font-bold uppercase text-muted-foreground">To</label><select className="h-11 rounded-xl border border-border/60 bg-background px-3 text-sm" value={opsForm.to_warehouse_id} onChange={(e) => setOpsForm({ ...opsForm, to_warehouse_id: e.target.value })}><option value="">Select destination</option>{warehouses.filter((w) => w.is_active).map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}</select></div>}<div className="grid gap-2"><label className="text-xs font-bold uppercase text-muted-foreground">{opsMode === 'adjust' ? 'Quantity Delta (+/-)' : 'Quantity'}</label><Input type="number" value={opsMode === 'adjust' ? opsForm.quantity_delta : opsForm.quantity} onChange={(e) => setOpsForm({ ...opsForm, [opsMode === 'adjust' ? 'quantity_delta' : 'quantity']: Number(e.target.value || 0) })} /></div>{opsMode === 'adjust' && <div className="grid gap-2"><label className="text-xs font-bold uppercase text-muted-foreground">Reason</label><Input value={opsForm.reason} onChange={(e) => setOpsForm({ ...opsForm, reason: e.target.value })} /></div>}{opsMode === 'receive' && <div className="grid gap-2"><label className="text-xs font-bold uppercase text-muted-foreground">Reference</label><Input value={opsForm.reference_id} onChange={(e) => setOpsForm({ ...opsForm, reference_id: e.target.value })} /></div>}<div className="grid gap-2 md:col-span-2"><label className="text-xs font-bold uppercase text-muted-foreground">Notes</label><Input value={opsForm.notes} onChange={(e) => setOpsForm({ ...opsForm, notes: e.target.value })} /></div></div><ModalFooter><Button variant="outline" onClick={() => setOpsOpen(false)} className="rounded-xl border-primary/40 bg-white text-primary hover:bg-primary hover:text-primary-foreground">Cancel</Button><Button variant="outline" onClick={submitOps} className="rounded-xl border-primary/40 bg-white text-primary hover:bg-primary hover:text-primary-foreground">Apply</Button></ModalFooter></ModalContent></Modal>
