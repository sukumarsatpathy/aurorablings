...
def _map_shipping_status_to_order_status(shipment_status: str) -> str | None:
    if shipment_status in {ShipmentStatus.PICKED_UP, ShipmentStatus.IN_TRANSIT, ShipmentStatus.OUT_FOR_DELIVERY}:
...
def preflight_validate_order(order: Order) -> PreflightResult:
    ...
    if order.status not in {OrderStatus.PAID, OrderStatus.PROCESSING, OrderStatus.SHIPPED}:
        errors.append("Order must be paid/processing/shipped for shipment creation.")

    if Shipment.objects.filter(order=order).exclude(status__in=[ShipmentStatus.CANCELLED, ShipmentStatus.FAILED]).exists():
...
def create_or_update_shipment_for_order(*, order_id: str, source: str = ShipmentEventSource.SYSTEM, force: bool = False) -> Shipment:
    ...
    if not created and shipment.external_shipment_id and not force:
        logger.info("shipment_create_skipped_existing", order_id=str(order.id), shipment_id=str(shipment.id))
        return shipment

    provider = resolve_provider()

    if get_shiprocket_config().get("serviceability_check_enabled", False):
    ...
    shipment.provider = ShipmentProvider.SHIPROCKET
...
def create_webhook_event(*, payload: dict, idempotency_key: str) -> ShipmentEvent:
    provider_status = str(payload.get("current_status") or payload.get("shipment_status") or payload.get("status") or "")
